...
leg = Legend(ax1, 
			np.array(legend)[[0,1,2,3,4,5]], 
			labels[[1,5,3,4,6,7]], 
			loc='center right', #loc='lower center', 
			bbox_to_anchor=(1.01,.5), #bbox_to_anchor=(.5,0),
...
# plt.savefig('car_constraint_values_wo_band.png', format='png', dpi=300)
# plt.show()
...
legend.append( mlines.Line2D([], [], color=derandom_color, linestyle='-',
	...
	# Pi_D
evaluation = np.mean(discounted_costs[:,0])
std = np.std(discounted_costs[:,0])
lines.append( ax1.plot(iterations, [evaluation]*len(iterations), color = pi_d_color,markersize=7,linestyle='-' , label='Exact') )
legend.append( mlines.Line2D([], [], color=pi_d_color, linestyle='-' ,
                      markersize=7, label='Percent of Track Covered') )
fill_betweens.append( ax1.fill_between(iterations,evaluation-std, evaluation+std, alpha = alpha, color = pi_d_color, zorder = 10) )

ax1.set_xlabel('Iteration (t)', fontsize=fontsize)
ax1.set_ylabel('Main Objective Value', color='k', fontsize=fontsize)
ax1.tick_params(axis='y', labelcolor='k')
ax1.set_ylim(bottom=-55, top=-15)
# ax1.set_ylim(bottom=20, top=55)
ax1.set_xlim(-.5, 25)
labels = np.array(['FQE', 'Algo 2', 'Mean', 'Regularized LSPI', 'Online-RL (no constraint)', 'Algo 2 (Derandomized)', r'$\pi_D$'])
# leg = Legend(ax1, np.array(legend), labels[[1,6]], loc='upper right', frameon=True, fontsize = legend_fontsize)
leg = Legend(ax1, 
			np.array(legend), 
			labels[[1,5,6]], 
			loc='lower center', 
...
plt.tick_params(axis='both', which='major', labelsize=major_tick_mark_size)
plt.tick_params(axis='both', which='minor', labelsize=8)
ax1.set_title('Main Objective - Accumulated Cost', fontsize=fontsize)
# plt.tight_layout()
plt.tight_layout(rect=[0,.15,1,1])
plt.savefig('car_main_value.png', format='png', dpi=300)
plt.show()
...
for idx in data['g_eval'].keys():
	...
	fill_betweens.append( ax.fill_between(iterations,evaluation-std, evaluation+std, alpha = alpha, color = car_color, zorder = 10) )
	lines.append( ax.plot(iterations, evaluation, color = car_color ,linestyle='-' , label=tex_labels[idx][1])  )
	legend.append( mlines.Line2D([], [], color=car_color, linestyle='-' ,
                          markersize=7, label=tex_labels[idx][1]) )

	# Derandomized
	lines.append( ax.plot(df_derandom['iterations'], df_derandom['g_%s_derandomized' % idx],color = derandom_color,linestyle='-',markersize=7, label='Exact') )
	legend.append( mlines.Line2D([], [], color=derandom_color, linestyle='-',
                          markersize=7, label=tex_labels[idx][2]) )

		# Pi_D
	evaluation = np.mean(discounted_costs[:,idx+1])
	std = np.std(discounted_costs[:,idx+1])
	lines.append( ax.plot(iterations, [evaluation]*len(iterations), color = pi_d_color,markersize=7,linestyle='-' , label=baseline_labels[idx][2]) )
	legend.append( mlines.Line2D([], [], color=pi_d_color, linestyle='-' ,
                      markersize=7, label=baseline_labels[idx][2]) )

	fill_betweens.append( ax.fill_between(iterations,evaluation-std, evaluation+std, alpha = alpha, color = pi_d_color, zorder = 10) )
	...
	if idx == 1:
		leg = Legend(ax, 
			...
			frameon=True, 
			fontsize = legend_fontsize)
		# leg = Legend(ax, legend, labels, title=label, loc='center left', frameon=False, bbox_to_anchor=(1, 0.5), fontsize = legend_fontsize-3)
		plt.setp(leg.get_title(),fontsize='%s' % legend_title_fontsize)
		ax.add_artist(leg)
...
axs[0].tick_params(axis='both', which='major', labelsize=major_tick_mark_size)
axs[1].tick_params(axis='both', which='major', labelsize=major_tick_mark_size)
axs[0].set_title('Constraints - Accumulated Cost', fontsize=fontsize)
# plt.tight_layout()
plt.tight_layout(rect=[0,.15,1,1])
plt.savefig('car_constraint_values.png', format='png', dpi=300)
plt.show()