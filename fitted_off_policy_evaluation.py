...
class CarFittedQEvaluation(FittedAlgo):
    ...
    def run(self, policy, which_cost, dataset, epochs=100, epsilon=1e-8, desc='FQE', g_idx=None, testing=False, **kw):
        # dataset is the original dataset generated by pi_{old} to which we will find
        # an approximately optimal Q
    ...
    def data_generator(self, dataset, training_idxs, fixed_permutation=False,  batch_size = 64):
        ...
        while True:
            ...
            # else:
            policy_action = policy([x_prime], x_preprocessed = True).reshape(-1)
